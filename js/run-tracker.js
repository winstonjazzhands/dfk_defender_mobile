(() => {
  ...
  function openQueuedRunsModal() {
    ...
    }).join('') : '<div class="queued-runs-empty">No stuck runs.</div>';
    ...
    backdrop.innerHTML = `
      <div class="queued-runs-modal">
        ...
        <div class="queued-runs-body">
          <div class="queued-runs-summary" data-queued-runs-status>${runs.length ? `${runs.length} pending run${runs.length === 1 ? '' : 's'} found.` : 'No stuck runs.'}</div>
          ${cards}
        </div>
